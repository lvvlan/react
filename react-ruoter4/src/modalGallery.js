...
class ModalSwitch extends React.Component {
  // 把一个位置（location）传给 <Switch/> 意味着路由会忽略当前的位置，
  // 并且使用被传入 prop 的位置（location）。
  // 「location state」属性使用户在弹窗（modal）里面访问路径「/images/2」，而
  // 不是在主页面上来访问这个路径，而且弹窗页面（modal）会把 gallery 页面挡住。
  // 通常，「/images/2」不会匹配到 gallery 的「/」， 而为了使两个页面都能渲染，
  // 我们要保存之前的位置，并且把这个位置传入Switch，然后就算我们已经转到
  // 「/images/2」这个位置了而Switch会以为当前位置还是「/」。

  previousLocation = this.props.location
  //每次路由 location和match都会改变 所以会触发组件的这个方法
  componentWillUpdate (nextProps, nextState) {
      const {location} = this.props;
      //console.log(this.props);
      //console.log(nextProps);
      // 如果 props.location 不是 modal 的话，就把 this.props.location 赋值给 previousLocation。
...
const Image = ({color}) => (
    <div style={{width: '100px', height: 400, background: color}}></div> 
);
